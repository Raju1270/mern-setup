...
export const VerifySignupOTPService = async (payload: OTPPayload) => {
  try {
    const response = await apiService.post('api/v1/auth/verify-signup', payload)
...
// TOGGLE MFA SERVICE
export const ToggleMFAService = async () => {
  try {
    const response = await apiService.post('api/v1/auth/mfa/toggle')
    ...
    showError('Failed to request MFA toggle')
    throw new Error('Failed to request MFA toggle')
  } catch (error) {
    showError(parseError(error))
    throw error
  }
...
// VERIFY MFA TOGGLE SERVICE
export const VerifyMFAToggleService = async (payload: OTPVerifyPayload) => {
  try {
    const response = await apiService.post('api/v1/auth/mfa/verify-toggle', payload)

    if (response?.success) {
      showSuccess(response.message || 'MFA updated successfully!')
      return response
    }

    showError('Failed to verify MFA toggle')
    throw new Error('Failed to verify MFA toggle')
  } catch (error) {
    showError(parseError(error))
    throw error
  }
